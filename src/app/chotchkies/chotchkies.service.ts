...
export class ChotchkiesService {
  ...
  createChotchkie(chotchkie: Chotchkie): Observable<Chotchkie> {
    return this.httpClient.post<Chotchkie>('/api/chotchkies', chotchkie).pipe(
      tap(() => {
  ...
  patchChotchkie(id: number, chotchkieData: any) {
    return this.httpClient
      .patch<Chotchkie>(`/api/chotchkies/${id}`, chotchkieData)
      .pipe(
        tap(() => {
          this._refreshNeeded$.next();
        })
  ...
  removeChotchkie(id: number) {
    return this.httpClient.delete(`/api/chotchkies/${id}`).pipe(
      tap(() => {
        this._refreshNeeded$.next();
      })
    );
