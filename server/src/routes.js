...
const createRoutes = () => {
  ...
  async function delayedSearchForChotchkieByName(req, res, name) {
    ...
    try {
      const result = await db.chotchkieByNameExists(
        safelyDecodeString(decodedName)
      );
      setTimeout(() => {
      ...
      res
        .status(404)
        .send(
          `Chotchkie with name ${decodedName} not found. Reason ${JSON.stringify(
            e
          )}`
        );
    }
  ...
  async function delayedHandleFuzzySearchTerm(req, res, searchTerm) {
    ...
    try {
      const result = await db.findChotchkiesByFuzzySearchTerm(
        safelyDecodeString(decodedSearchTerm)
      );
      setTimeout(() => {
        res.status(200).send(result);
      }, Math.random() * 3000);
      ...
      res
        .status(404)
        .send(
          `Chotchkie with search term ${decodedSearchTerm} not found. Reason ${JSON.stringify(
            e
          )}`
        );
    }
  ...
  router.patch("/chotchkies/:id", async (req, res) => {
    try {
      ...
      if (!chotchkieId) {
        ...
        const updatedChotchkie = await db.patchChotchkie(
          chotchkieId,
          targetChanges
        );
