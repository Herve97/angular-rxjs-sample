class Database {
  constructor(seed) {
    this.database = seed;
    this.nextKey = this.database.chotchkies.reduce((acc, curr) => {
  ...
  chotchkieByNameExists(term) {
    const result = this.database.chotchkies.findIndex((chotchkie) => {
      console.log(
        `comparing ${term} to ${chotchkie.name} in ${JSON.stringify(term)}`
      );
  ...
  findChotchkiesByFuzzySearchTerm(term) {
    const matches = this.database.chotchkies.filter(
      (chotchkie) =>
  ...
  updateChotchkie(id, props) {
    const chotchkieIndex = this.database.chotchkies.findIndex(
      (c) => c.id === id
    );
    if (chotchkieIndex > -1) {
   ...
   *
   * TODO: real database, real locking. This.is.a.toy.
   */
  async patchChotchkie(id, props) {
    ...
    if (props.purchasedQuantity) {
      const newQuantity =
        currentChotchkie.quantityOnHand - props.purchasedQuantity;
      if (newQuantity < 0) {
